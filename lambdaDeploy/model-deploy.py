...
def lambda_handler(event, context):
    ...
    register_model_response = register_model(container, model_data_url, best_training_job)

    print("Save results of model (tunning job) to S3")
    training_job_description = sagemaker.describe_training_job(TrainingJobName=best_training_job)
    objective_metric_value = [
        element
        for element in training_job_description["FinalMetricDataList"]
        if element["MetricName"] == "ObjectiveMetric"
    ][0]["Value"]
...
def register_model(container, model_data_url, best_training_job):
    ...
    create_mode_package_response = sagemaker.create_model_package(**create_model_package_input_dict)
...
def create_model(name, container, model_data_url):
    ...
    try:
        sagemaker.create_model(
            ModelName=name,
            PrimaryContainer={"Image": container, "ModelDataUrl": model_data_url},
...
def create_endpoint_config(name):
    ...
    try:
        sagemaker.create_endpoint_config(
            ...
            ProductionVariants=[
                {"VariantName": "prod", "ModelName": name, "InitialInstanceCount": 1, "InstanceType": INSTANCE_TYPE}
            ],
        )
    except Exception as e:
        print(e)
